...
import datasetJson from "../dataset/webdevelopment.json";

const ITEMS_PER_PAGE = 5;

const Webdevelopmentbot = () => {
  const [input, setInput] = useState("");
  const [chatHistory, setChatHistory] = useState([
    {
      sender: "bot",
      text: "Ask a question about HTML, CSS, or JavaScript. Type 'help' to see all web dev topics."
    }
  ]);
  ...
  const findBestTopic = (rawQuestion) => {
    const q = rawQuestion.toLowerCase().trim();
    if (!q) return null;

    // split into words: ["what","is","javascript"]
    const words = q.split(/[^a-z0-9+.#]+/i).filter(Boolean);

    let best = null;
    let bestScore = 0;

    datasetJson.data.forEach(topic => {
      let score = 0;

      topic.keywords.forEach(kw => {
        const k = kw.toLowerCase();
        // keyword matches if appears as a whole word OR as substring
        if (words.includes(k) || q.includes(k)) {
          score += 1;
        }
      });
  ...
  const handleAsk = () => {
    ...
    if (qLower === "help") {
      ...
      addMessage(
        "bot",
        "Here are all available Web Development concepts. Click a topic button to view the concept."
      );
    } else if (helpMode) {
      ...
      if (!isNaN(num) && num >= 1 && num <= datasetJson.data.length) {
        ...
        const found = findBestTopic(question);
        if (found) {
      ...
      <main className="flex flex-1 flex-col gap-8 px-4 py-6 md:px-10 md:py-8">
        ...
        <div className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
          <div>
            ...
            <p className="mt-2 text-sm text-sky-300 md:text-base">
              Bot is in basic version. Ask concepts from HTML, CSS, and JavaScript or type "help".
            </p>
          </div>

          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 items-center justify-center rounded-full border-2 border-sky-400 bg-[radial-gradient(circle_at_30%_0,#22d3ee,#0f172a_60%)] text-3xl md:h-20 md:w-20">
              💻
            </div>
            <div>
              ...
              <div className="text-xs text-slate-400 md:text-sm">
                Ask anything about your frontend syllabus.
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-1 flex-col overflow-hidden rounded-3xl border border-slate-500/30 bg-slate-900/95 p-4 shadow-[0_18px_60px_rgba(15,23,42,0.9)] md:p-6">
              ...
              return (
                ...
                >
                  ...
                  >
                    {msg.text}
                    {msg.text.includes(
                      "Here are all available Web Development concepts"
                    ) && renderHelpPage()}
                  </div>
                </div>
              );
          ...
          <div className="mt-4 flex items-center gap-3 border-t border-indigo-800/70 pt-3">
            <div className="flex flex-1 items-center gap-2 rounded-full border border-sky-400 bg-[radial-gradient(circle_at_0_0,rgba(56,189,248,0.25),rgba(15,23,42,0.95))] px-4 py-2">
              <input
                value={input}
                onChange={e => setInput(e.target.value)}
                onKeyDown={handleKeyPress}
                placeholder="Type your web dev question..."
...
export default Webdevelopmentbot;
