...
const DashboardLayout = () => {
  ...
  // NEW: compute days in this month & first weekday
  const daysInMonth = new Date(currentDateTime.getFullYear(), currentDateTime.getMonth() + 1, 0).getDate(); // [web:61][web:63]
  const firstDayOfMonth = new Date(currentDateTime.getFullYear(), currentDateTime.getMonth(), 1).getDay();   // 0=Sun..6=Sat [web:74]
      ...
      <div className="flex-1 flex flex-col bg-[#141416] rounded-xl overflow-hidden shadow-2xl shadow-indigo-900/50">
        ...
        <header className="flex justify-between items-center p-4 bg-gray-900 border-b border-gray-800">
          <div className="flex items-center space-x-4">
            ...
            {/* Search Bar - Hidden on small screens; now functional */}
        ...
        <main className="p-4 md:p-6 flex-1 overflow-y-auto">
          ...
          <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-6 mb-8">
            ...
            {/* Hero Banner (Featured Course Carousel) */}
            ...
            <div className="w-full md:w-1/3 bg-gray-900 rounded-xl p-4 shadow-inner shadow-indigo-900/30 border border-gray-800 mt-4 md:mt-0">
              ...
              <div className="grid grid-cols-7 gap-1 text-center text-xs md:text-sm mb-4">
                ...
                {/* Fillers for the start of the month based on actual firstDayOfMonth */}
                {Array.from({ length: firstDayOfMonth }).map((_, index) => (
                  <div key={`filler-${index}`} className="p-1"></div>
                ))}

                {/* Rounded calendar days for this real month length */}
                {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day, index) => (
                  <div 
                    key={index} 
                    className={`w-8 h-8 mx-auto flex items-center justify-center rounded-full font-medium text-gray-300 hover:bg-gray-800 
          ...
          {/* Bottom Row: Course Cards and Notification Widget */}
