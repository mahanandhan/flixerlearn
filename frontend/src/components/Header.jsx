...
const Sidebar = ({ isOpen, toggleSidebar }) => (
  ...
  >
    ...
    <nav className="space-y-6">
      <Link
        to="/"
        className="p-3 rounded-xl bg-indigo-700 text-white shadow-lg shadow-indigo-700/50 cursor-pointer block"
      >
...
// mark your own browser once in DevTools: localStorage.setItem("flixerIsOwner", "true");
const isOwnerBrowser = () => {
  if (typeof window === "undefined") return false;
  return localStorage.getItem("flixerIsOwner") === "true";
};
...
const DashboardLayout = () => {
  ...
  // VIEW LOGIC: per-day, per-browser, skip owner, no double count on refresh
  useEffect(() => {
    if (typeof window === "undefined") return;

    const key = "flixerVisitorInfo";
    ...
    if (!data || data.date !== todayStr) {
      data = { date: todayStr, hasVisited: false, count: 0 };
    }

    // If it's your browser, do not increment, only show current count
    if (isOwnerBrowser()) {
      setTodayVisitorCount(data.count);
      localStorage.setItem(key, JSON.stringify(data));
      return;
    }

    // Other users: first visit today increments by 1
    if (!data.hasVisited) {
      data.count += 1;
        ...
        <header className="flex justify-between items-center p-4 bg-gray-900 border-b border-gray-800">
          ...
          <div className="flex items-center space-x-4">
            <div className="hidden md:block text-xs text-gray-400">
              Today&apos;s unique visits (this browser record):{" "}
              <span className="text-indigo-400 font-semibold">
                {todayVisitorCount}
              </span>
            </div>
            <div className="flex items-center space-x-2 bg-gray-800 p-1 rounded-full cursor-pointer">
              {/* SettingsIcon / profile can go here */}
            </div>
          </div>
        </header>

        <main className="p-4 md:p-6 flex-1 overflow-y-auto">
          {/* Big stat card above featured courses */}
          <div className="mb-6">
            <div className="bg-gradient-to-r from-indigo-600 via-fuchsia-600 to-cyan-500 rounded-2xl p-[1px] shadow-xl shadow-indigo-900/40">
              <div className="bg-gray-900 rounded-2xl px-5 py-4 md:px-7 md:py-5 flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs md:text-sm text-gray-400">
                    Counted once per user per day (owner browser not included)
                  </p>
                </div>
                <div className="hidden md:flex items-center justify-center w-16 h-16 rounded-2xl bg-white/10 border border-white/10">
                  <svg
                  ...
                  >
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth="2"
                      d="M3 13a4 4 0 014-4h3m0 0L9 7m1 2l-1 3m5-3h3a4 4 0 014 4m-7-3l1-3m-1 3l-1 3"
                    />
                  </svg>
                </div>
              </div>
          ...
          <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-6 mb-8">
            ...
            <div className="w-full md:w-1/3 bg-gray-900 rounded-xl p-4 shadow-inner shadow-indigo-900/30 border border-gray-800 mt-4 md:mt-0">
              ...
              <div className="mb-6 bg-gray-800 p-3 rounded-lg border border-indigo-700/50">
                <div className="text-white font-medium text-xl md:text-2xl mb-1">
                  {formattedTime}
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              >
                {(filteredCourses.length > 0 ? filteredCourses : courses).map(
                  (course, index) => (
                    ...
                    >
                      <div
                        className={`absolute inset-0 -m-px rounded-xl bg-linear-to-r ${getCardGradient(
                          index
                        )} opacity-30 blur-sm`}
                      ></div>
            ...
            <div className="col-span-1 md:col-span-1">
              <h3 className="text-xl font-semibold text-gray-300 mb-4 mt-4 md:mt-0">
                Notifications
              </h3>
